...
# =============== 选择模式 ===============
class CompressProfileSelector(tk.Toplevel):
	def __init__(self, master):
		...
		pad = 14
		ttk.Label(self, text="请选择视频压缩的编码器/模式：", font=("Microsoft YaHei", 11)).pack(padx=pad, pady=(pad, 6))

		self.mode = tk.StringVar(value="cpu")   # 默认 CPU x264
		self.hw   = tk.StringVar(value="nvenc") # 默认 NVIDIA

		box = ttk.Frame(self); box.pack(padx=pad, pady=6, fill="x")
		ttk.Radiobutton(box, text="① CPU（x264）", variable=self.mode, value="cpu").pack(anchor="w", pady=2)
		ttk.Radiobutton(box, text="② 硬件加速（H.264）", variable=self.mode, value="hw").pack(anchor="w", pady=2)

		hwfrm = ttk.Frame(self); hwfrm.pack(padx=pad, pady=(8, 6), fill="x")
		ttk.Label(hwfrm, text="显卡：").grid(row=0, column=0, sticky="w")
		hwopts = ttk.Frame(hwfrm); hwopts.grid(row=0, column=1, sticky="w", padx=(6,0))
		ttk.Radiobutton(hwopts, text="NVIDIA (h264_nvenc)", variable=self.hw, value="nvenc").pack(side="left", padx=(0,8))
...
def choose_compress_profile() -> Optional[dict]:
	root = tk.Tk(); root.withdraw()
	dlg = CompressProfileSelector(root)
...
class SplitProfileSelector(tk.Toplevel):
	def __init__(self, master):
		super().__init__(master)
		self.title("选择拆分编码模式")
		self.resizable(False, False)
...
def ffprobe_pix_fmt(path: Path) -> Optional[str]:
	try:
		out = subprocess.check_output(
...
def split_worker(in_path: Path, split_points: List[float], out_dir: Path, enc_profile: dict, q: Queue,
	...
	src_pix = ffprobe_pix_fmt(in_path) or "yuv420p"

	mode = (enc_profile or {}).get("mode", "vis")
...
def main():
	...
	if mode in {"quality", "size", "audio"}:
		...
		if mode in {"quality", "size"}:
			...
			# ——新增：选择压缩编码器（CPU 或 硬件加速）——
			profile = choose_compress_profile()
			if not profile:
				messagebox.showinfo("提示", "未选择编码器，已取消。")
				return

			# 按模式重写 plan.args（质量档与大小档分别给出合理参数）
			try:
				if profile.get("mode") == "hw":
					hw = profile.get("hw", "nvenc")
					if mode == "quality":
						# 画质优先（≈CRF18 的主观质量）
						if hw == "nvenc":
							plan.args = ["-c:v", "h264_nvenc", "-preset", "p5", "-tune", "hq", "-rc", "vbr", "-cq",
										 "18", "-b:v", "0", "-maxrate", "0",
										 "-pix_fmt", "yuv420p", "-c:a", "copy", "-movflags", "+faststart"]
						elif hw == "qsv":
							plan.args = ["-c:v", "h264_qsv", "-global_quality", "18", "-look_ahead", "1", "-g", "240",
										 "-bf", "3",
										 "-pix_fmt", "nv12", "-c:a", "copy", "-movflags", "+faststart"]
						else:  # amf
							plan.args = ["-c:v", "h264_amf", "-quality", "quality", "-rc", "vbr", "-qp_i", "18",
										 "-qp_p", "18", "-qp_b", "20", "-g", "240", "-bf", "3",
										 "-pix_fmt", "yuv420p", "-c:a", "copy", "-movflags", "+faststart"]
					else:
						# 体积优先（≈CRF26~28 的主观强度）
						if hw == "nvenc":
							plan.args = ["-c:v", "h264_nvenc", "-preset", "p4", "-rc", "vbr", "-cq", "26", "-b:v", "0",
										 "-maxrate", "0",
										 "-pix_fmt", "yuv420p", "-c:a", "aac", "-b:a", "128k", "-movflags",
										 "+faststart"]
						elif hw == "qsv":
							plan.args = ["-c:v", "h264_qsv", "-global_quality", "26", "-look_ahead", "1", "-g", "240",
										 "-bf", "3",
										 "-pix_fmt", "nv12", "-c:a", "aac", "-b:a", "128k", "-movflags", "+faststart"]
						else:  # amf
							plan.args = ["-c:v", "h264_amf", "-quality", "speed", "-rc", "vbr", "-qp_i", "26", "-qp_p",
										 "26", "-qp_b", "28", "-g", "240", "-bf", "3",
										 "-pix_fmt", "yuv420p", "-c:a", "aac", "-b:a", "128k", "-movflags",
										 "+faststart"]
				else:
					# 保持原 CPU x264（build_plan 已给出）
					pass
			except Exception as e:
				messagebox.showerror("错误", f"编码器参数设置失败：{e}")
				return
		...
		# ——新增：选择编码模式——
		profile = choose_split_profile()
		if not profile:
			messagebox.showinfo("提示", "未选择编码模式，已取消。")
			return
		...
		# 这里把 profile 作为新的参数传给 worker
		worker_args_base = (in_path, split_points, out_dir, profile)
